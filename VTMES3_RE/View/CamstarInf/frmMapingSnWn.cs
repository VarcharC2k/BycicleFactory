...
{
    public partial class frmMapingSnWn : DevExpress.XtraEditors.XtraForm
    {
        // 템플릿 파일 경로 
        string folderName = Application.StartupPath + @"\Templates\MappingSnWn";
        string fileName = "MappingContainer.xlsx";
        ...
        private void DisplayData(DataView Dv,int crd)
        {
            ...
            object[] arrobj = MasterdataTable.Select().Select(x => x["AY_Name"]).ToArray();
            string[] arrstr = arrobj.Cast<string>().ToArray();

            object[] proobj = MasterdataTable.Select().Select(x => x["QC_Name"]).ToArray();
            string[] prostr = proobj.Cast<string>().ToArray();
            ...
            for(int k =1; k < excelSheetControl.Document.Worksheets[0].GetDataRange().RowCount; k++)
            {
                for (int i = 0; i < arrstr.Length; i++)
                {
                    ...
                    excelSheetControl.Document.Worksheets[0].Cells[crd, l].SetValue(prostr[i].ToString());
        ...
        private void cmdSearch_ElementClick(object sender, DevExpress.XtraBars.Navigation.NavElementEventArgs e)
        {
            ...
            if (isSubmit)
            {
                MessageBox.Show("재조회시 닫기 버튼을 누른 후 다시 조회 하여 주시길 바랍니다.", "알림", MessageBoxButtons.OK, MessageBoxIcon.Information);
                return;
            }
            ...
            for(int i=1; i < excelSheetControl.Document.Worksheets[0].GetDataRange().RowCount; i++)
            {
                ...
                co = work.GetFTContainerName(excelSheetControl.Document.Worksheets[0].GetCellValue(0, i).TextValue);
                if (co.Table.Rows.Count <= 0)
                {
                    excelSheetControl.Document.Worksheets[0].Cells[i, 1].SetValue("해당 " + excelSheetControl.Document.Worksheets[0].GetCellValue(0, i).TextValue + " Serial Number에 매칭되어 있는 Container Name이 조회되지 않습니다. 다시한번 확인해주세요.");
                }        

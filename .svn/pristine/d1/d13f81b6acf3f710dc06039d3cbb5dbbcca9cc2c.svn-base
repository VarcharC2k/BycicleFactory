...
{
    public partial class frmExecTaskByTftUnit : DevExpress.XtraEditors.XtraForm
    {
        ...
        private void frmExecTaskByTftUnit_Load(object sender, EventArgs e)
        {
        ...
        private void btnSearch_Click(object sender, EventArgs e)
        {
            ...
            // string testString = "";
            int cellX = 1;//XtraSpreadsheet 의 엑셀 칼럼명 대괄호 오류시 위치 정보로 수정

            foreach (DataRowView drv in collectionView)
            {
                if (drv["DataType"].ToString() == "5" && drv["NamedObjectGroupName"].ToString() != "")
                {
                    // testString = drv["DataPointName"].ToString();
                    //CellRange comboBoxRange = excelSheetControl.Document.Worksheets[0][string.Format("DataPoint[{0}]", drv["DataPointName"].ToString())];
                    CellRange comboBoxRange = excelSheetControl.Document.Worksheets[0].Rows[1][cellX].GetRangeWithAbsoluteReference();// 엑셀 칼럼명에 대괄호가 2번 들어가면 오류 발생 하므로, 칼럼 번호로 변경
                    //참고 : excelSheetControl.Document.Worksheets[0].Range["$B$2:$B$3"] // 열행 Range["$B$2:$B$3"]
        ...
        private void txtContainerId_EditValueChanged(object sender, EventArgs e)
        {
            ...
            DataCollectionLookUpEdit.Properties.DataSource = work.GetDataCollectionTFT("TFT_", txtContainerId.Text.ToString());
            DataCollectionLookUpEdit.Properties.DisplayMember = "명칭";
            DataCollectionLookUpEdit.Properties.ValueMember = "코드";
            DataCollectionLookUpEdit.EditValue = null;
        }
        ...
        private void btnSchPQC_Click(object sender, EventArgs e)
        {
            if (dataCollectionId == "") return;
            ...
            try
            {
                ...
                if (arrTask[1].Contains("QC"))
                {
                    ...
                    successCnt = WrGlobal.Camster_Common.PQCDataOpen(DataCollectionLookUpEdit.EditValue.ToString(), dataTable, false);
